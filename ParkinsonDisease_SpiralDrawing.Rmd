...
patients <- patients %>% mutate(areaT1 = NA)
patients <- patients %>% mutate(sdT0 = NA)
patients <- patients %>% mutate(sdT1 = NA)
...
for(test in seq(0, by=1, length=2)){
  ...
  for(p in seq(1, by=1, length=nrow(patients))){
   ...
   x <- b*t * (cos(t)) + temp_df[1,]$X
   y <- b*t * -(sin(t)) + temp_df[1,]$Y
   archimedeanSpiral = data.frame(x,y)
   archimedeanSpiral <- archimedeanSpiral %>% mutate(nr=row_number())
...
for(test in seq(0,by=1,length=2)){
    ...
    for(index in seq(1, by=1, length=nrow(patients))){
      ...
      for(row in seq(2, by=1, length=nrow(temp))){
        ...
        temp[row,]$Vt <- sqrt((temp[row,]$X - temp[row-1,]$X)^2 + (temp[row,]$Y - temp[row-1,]$Y)^2)
        temp[row,]$Dist <- temp[row - 1,]$CumulDistXY + temp[row,]$DistXY
      }
