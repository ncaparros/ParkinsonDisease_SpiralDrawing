...
It was decided to perform three handwriting tests on a graphic tablet (Wacom Cintiq 12WX graphics, see Figure 3). The tablet would measure several parameters as : the coordinates (x-y-z) of the pen on the tablet, the pressure over the screen, the grip angle on the pen at regular time intervals. A software was developped in order to test the coordination of the patient.
...
```
The Figure \ref{fig:figs} (left), shows an Archimedean spiral similar to the one used in the SST and DST. Here, $a=0$, $b=10.61$, and the length is $6\pi$ ($t$ ranges from $0$ to $6\pi$). This means that the spiral does three complete turns.

The Figure \ref{fig:figs} (right), shows an Archimedean spiral with a length of only $\frac{\pi}{3}$, to show the different parameters. Here, $a=20$ and $b=30$, and we have the following equations :
$$\left\{
    \begin{array}{l}
      x_b(t)= bt*cos(t)\\
      y_b(t)= bt*sin(t)\\
      b(t) =\sqrt{x_b(t)² + y_b(t)²} = \sqrt{(bt*cos(t))²+(bt*sin(t))²} = \sqrt{b²t²(cos²(t)+sin²(t))}=\sqrt{b²t²}=bt\\
      b(t=0) = b*t=0
    \end{array}
  \right.$$
...
The timestamp columns seemed pretty obscur, and trying to parse it into a readable date would produce either an impossible date (`make_date` or  `make_datetime`) or NA values (`dym`, `mdy_hms`, `as.Date`,...).
...
A quick look at the test results (the drawings) showed that the samples were not all consistent in term of X-Y ratio (see Plot \ref{fig:figs4}). Seven samples can be seen with a X-Y ratio of almost 2 instead of 1. With the Archimean spiral formula in mind, it seemed odd that a small part of the patients would have been given a different test with a different drawing to follow. 

```{r ploting X-Y for SST, fig.cap="\\label{fig:figs4}SST and DST samples", echo=FALSE}
...
Another odd detail was a spacial gap between patterns with the same ratio (see Plot \ref{fig:figs5}). One of the control sample seemed to have been shifted.

```{r ploting X-Y for Test0 and controls, fig.cap="\\label{fig:figs5}SST samples (controls)", echo=FALSE}
...
```{r ploting ratios, fig.cap="\\label{fig:figs7}X-Y ratios before cleaning", echo=FALSE, message=FALSE, warning=FALSE}
...
ratiosDf %>% 
  ...
  theme_solarized()
```
We can indeed see that even if most ratios are around 1.3, some outliers are around 2 (see Figure \ref{fig:figs7}). Even the initial point of the drawing was different (see Figure \ref{fig:figs8}).
...
This model took into account the assumption of which the very first values X,Y were, in general, the closest to the Archimedean Spiral when $Timestamp = 0$. $X_{min}$ and $X_{max}$ (and respectively $Y_{min}$ and $Y_{max}$) were approximations of the very tip of the drawings (see Figure \ref{fig:figs9}).
...
Combining those three models gave for patient $p$, test $t$ and spiral length $L$ equal spiral height (see Figure \ref{fig:figs10}:
...
The logic behind the algorithm was quite simple. I defined four variables in the `patients` dataframe :
...
For each unique point on the drawing (SST and DST), I determined the closest point on the Archimedean spiral using `gDistance` of the `rgeos` package and `which.min`. I then calculated the distance using :
...
Initial study "Improved spiral test using digitized graphics tablet for monitoring
Parkinson’s disease" by Muhammed Erdem Isenkula, Betul Erdogdu Sakarb, Olcay Kursuna : https://pdfs.semanticscholar.org/83f6/c11e9ebab1dea4aa3c5a7f9eb692f33d17c1.pdf


Data set, UCI : https://archive.ics.uci.edu/ml/datasets/Parkinson+Disease+Spiral+Drawings+Using+Digitized+Graphics+Tablet
