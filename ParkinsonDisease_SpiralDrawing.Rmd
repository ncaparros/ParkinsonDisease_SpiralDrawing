...
completeDf %>% 
  ...
  geom_point(aes(X,Y, color=patientID), 
             show.legend = FALSE) + 
  ggtitle("Different ratios for Test 0 samples")
```
...
completeDf %>% 
  ...
  geom_point(aes(X,Y, 
                 color=patientID), 
             show.legend = FALSE) + 
  ggtitle("Spatial gap for Test 0 samples")
```
...
completeCalibratedDf %>% 
  ...
  geom_point(aes(X,Y, 
                 color=patientID), 
             show.legend = FALSE) + 
  ggtitle("Calibrated samples for Test 0 and 1") + 
...
**Akinesia** : Loss of motor function, resulting in impaired muscle movement.

**Basal ganglia** : Group of structures situated at the base of the forebrain and top of the midbrain. It is strongly interconnected with the cerebral cortex and is associated with control of motor movements, cognition, emotion (non-exhaustive). The *substancia nigra** is a grey matter portion of the basal ganglia.

![Basal ganglia (within red square, source : https://www.neuroscientificallychallenged.com)](img/basal-ganglia.jpg)

**Dopaminergic** : Involving or transmitting dopamine (both hormone and neurotransmitter, plays a role in motor control, and mood)
