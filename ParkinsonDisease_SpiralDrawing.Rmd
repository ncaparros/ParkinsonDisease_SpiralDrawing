...
if(!require(rt)) install.packages("dplyr", repos = "http://cran.us.r-project.org")
...
Parkinson's disease, sometimes abreviated as PD, is a long-term neurodegenerative disorder. Its cause is unknown, though it is believed to involve genetic (as relatives tend to contract the disease), and/or environmental factors (as pesticides). 
...
The motor symptoms of the Parkinson's disease (parkinsonian syndrome) are caused by the death of cells, more precisely dopaminergic neurons, in the *substantia nigra* (a region of the midbrain, see Figure \ref{fig:figs}, left). The *substantia nigra* is a structure divided into to parts : the *pars reticula* and the *pars compacta* (see Figure \ref{fig:figs}, left). It is the part of the brain that plays an important role in reward-seeking, learning and movement. 
...
The lack of dopamine (due to the death of those cells, and therefore induces a smaller substancia nigra than on a healthy subject, see Figure \ref{fig:figs}, right) provokes emotional troubles, and since the downsized *substancia nigra* is connected to the motor cortex (via the *pars reticula*, see Figure \ref{fig:figs}, left), it causes the parkisonian syndromes.

The Figure \ref{fig:figs} (left) shows a lateral cross-section of a brain. The red arrows represent the dopamine's exchanges between the *pars reticula*. 

The Figure \ref{fig:figs} (right) shows the lack of dopaminergic neurons in a brain of a person affected by Parkinson's disease compared to a healthy brain.

```{r, echo=FALSE,out.width="49%", out.height="20%",fig.cap="\\label{fig:figs}Lateral cross-section of the brain (left, source : http://www.neuroconvention.com/), *Substancia nigra* differences between a healthy brain and a Parkinson's brain (right, source : https://scienceofparkinsons.com/) ",fig.show='hold',fig.align='center'}
...
It was decided to perform three handwriting tests on a graphic tablet (Wacom Cintiq 12WX graphics, see Figure \ref{img}). The tablet would measure several parameters as : the coordinates (x-y-z) of the pen on the tablet, the pressure over the screen, the grip angle on the pen at regular time intervals. A software was developped in order to test the coordination of the patient.

```{r pressure, echo=FALSE, fig.cap="\\label{img}Wacom Cintiq 12WX graphics, source : https://www.bhphotovideo.com/", out.width = '50%', fig.align='center'}
knitr::include_graphics("img/wacom.jpg")
```
...
$$r = a + b\theta$$
With $r$ the radius, $a$ real number turning the spiral, $b$ real number controling the distance between successive turns, and $\theta$ the angle velocity. 
The spiral is the locus of points corresponding to the locations over time of a point moving away from a fixed point with constant speed along a line that rotates with constant angular velocity.
...
```{r archimedean spiral, echo=FALSE, fig.align="center", fig.cap="\\label{fig:figs3}Archimedean spirals"}
...
```
The Figure \ref{fig:figs3} (left), shows an Archimedean spiral similar to the one used in the SST and DST. Here, $a=0$, $b=10.61$, and the length is $6\pi$ ($t$ ranges from $0$ to $6\pi$). This means that the spiral does three complete turns.

The Figure \ref{fig:figs3} (right), shows an Archimedean spiral with a length of only $\frac{\pi}{3}$, to show the different parameters. Here, $a=20$ and $b=30$, and we have the following equations :
$$\left\{
    \begin{array}{l}
      x_b(t)= bt*cos(t)\\
      y_b(t)= bt*sin(t)\\
      b(t) =\sqrt{x_b(t)^2 + y_b(t)^2} = \sqrt{(bt*cos(t))^2+(bt*sin(t))^2} 
      \\ = \sqrt{b^2t^2(cos^2(t)+sin^2(t))}=\sqrt{b^2t^2}=bt\\
      b(t=0) = b*t=0
    \end{array}
  \right.$$
...
## Dataset overview\label{dataset_overview}
...
Once the data downloaded and the data frame built (see \ref{dataset_overview} *Dataset overview*), the first step is to add a random identifier to each patient and to note if he has Parkinson's disease or not. The random identifier has been chosen because :
...
## Data cleaning \label{section_data_cleaning}
...
To be able to use the timestamp more easily, and mostly because we do not know its unit (probably milliseconds but we cannot know for sure), I substract the first timestamp of every couple test/patient to all the timestamp values, making the first value 0. To do this I have to create a new empty data frame, and two for loops : one for the tests (0 to 2) and one for the patients (0 to `r nrow(patients)`). Inside the loops, I would get the values of the current patient for the current test, arranged by ascending timestamp, and the first value would be the initial timestamp value. Then, every timestamp would be mutated as $timestamp_i = timestamp_i - timestamp_0$. The mutated data frame would then be merge (`rbind`) into the final data frame.
...
### Calibrating the samples\label{calibrating_samples}

A quick look at the test results (the drawings) shows that the samples are not all consistent in term of X-Y ratio (see Plot \ref{fig:figs4}, left). Seven samples can be seen with a X-Y ratio of almost 2 instead of 1. With the Archimean spiral formula in mind, it seems odd that a small part of the patients would have been given a different test with a different drawing to follow. 

```{r ploting X-Y for SST, fig.cap="\\label{fig:figs4}SST samples", echo=FALSE}
#Ploting the samples for the SST by patient. It shows the different ratios in the records.
ratiosplot <- completeDf %>% 
  filter(TestID == 0) %>%
  ggplot() +
...
ggarrange(ratiosplot, spatialGapPlot)
```

Another odd detail is a spacial gap between patterns with the same ratio (see Plot \ref{fig:figs4}, right). One of the control sample seems to have been shifted.


This disparity between the samples prevented me from trying to analyse the spacial differences between the generated Archimedean spiral and the patient's drawing. I checked if the timestamp data seemed affected or not (see Plot \ref{fig:figs6}). It seemed as only the X and Y values were sometimes off.

```{r checking timestamps, fig.cap="\\label{fig:figs6}SST sample times", echo=FALSE, message=FALSE, warning=FALSE}
...
To calibrate the X and Y values, the ratios $\frac{X}{Y}$ are calculated and stored in a new data frame *ratiosDf*, by patient and test. This data frame contains the ratio between X and Y as $$ratio=\frac{X_{max} - X_{min}}{Y_{max}-Y_{min}}$$

```{r calculate ratios, echo=FALSE, message=FALSE, warning=FALSE}
...
We can indeed see that even if most ratios are around 1.3, some outliers are around 2 (see Figure \ref{figure7}).


```{r ploting ratios, echo=FALSE, fig.cap="\\label{figure7} X-Y ratios before cleaning",message=FALSE, warning=FALSE}
...
```
 ...
 Even the initial point of the drawing is different (see Figure \ref{fig:figs8}).

```{r starts of drawings for SST, fig.cap="\\label{fig:figs8}SST starting point", echo=FALSE}
...
* $X_{param} = \overline{X}$ and $Y_{param} = \overline{Y}$
* $X_{param} = X_{median}$ and $Y_{param} = Y_{median}$
* $X_{param} = \overline{X_{Y = Y_{median}}}$ and $Y_{param} =\overline{Y_{X = X_{median}}}$
* $X_{param} = \overline{X_{Y = Y_{mean}}}$ and $Y_{param} = \overline{Y_{X = X_{mean}}}$
* $X_{param} = \overline{X_{Y = Y_{Timestamp = 0}}}$ and $Y_{param} = \overline{Y_{X = X_{Timestamp = 0}}}$
...
This model takes into account the assumption of which the very first values X,Y are, in general, the closest to the Archimedean Spiral when $Timestamp = 0$. $X_{min}$ and $X_{max}$ (and respectively $Y_{min}$ and $Y_{max}$) are approximations of the very tip of the drawings (see Figure \ref{fig:figs9}).

```{r modeling the Spiral and X-Y params, fig.cap="\\label{fig:figs9}Archimedean spiral", echo=FALSE}
...
Combining those three models gives for patient $p$, test $t$ and spiral length $L$ equal spiral height (see Figure \ref{fig:figs10}):
...
```{r calibrating X and Y, fig.cap="\\label{fig:figs10}Calibrated SST and DST samples", echo=FALSE}
...
This section describes the analysis of the different tests in the dataset. Since there is no prediction here, and some additionnal information (calculated and determined values) are to be noted in the `patients` data frame, the analysis is performed on the whole datas.

### Global analysis

The very first point to stress out is that every patient did not perform the three tests (see Figure \ref{fig:figs11}).

```{r tests taken, fig.cap="\\label{fig:figs11} Tests taken", echo=FALSE, warning=FALSE}
...
This would probably alter the accuracy of the future predictions. The original study based its analysis on the combination of SST and DST, and even though every patient took the SST (Static Spiral Test), some DST (Dynamic Spiral Test) were missing from the dataset. This issue is detailed and handled on section *Test inconsistency* (see \ref{sect_test_inconsistency}).

### Creating training and testing sets

In order to calibrate the Archimedean spiral, I need to create training and testing sets. Since we have very little values, I need to make sure I have control patients in both sets, as well as patients (control and not) who had taken the Stability on Certain Point Test.
I create several sub-datasets as :
...
### Defining the Archimedean spiral\label{arch_spiral}
...
I define $a = 0$ and determine that the spiral ranged from $0$ (the origin of the spiral) to $6\pi$ (the last point of the spiral). This means that the spiral did three complete turns. I had to make a slight modification so the Archimedean spiral would match the drawings, as :
...
I fix the maximum size of the spiral to 200 (see \ref{calibrating_samples} *Calibrating the samples*), so I have the equation :
...
Which means that $b=\frac{200}{6\pi} \approx 10.61$. I also need to slide the spiral, with $x_{adjust} = \overline{x(t=0, isPwp = FALSE)}$ and $y_{adjust}= \overline{x(t=0, isPwp = FALSE)}$ so that it is overlapping the drawings. The equation is then :
...
```{r calculating distance between Archimedean Spiral and drawing0, fig.cap="\\label{fig:figs12} SST - Archimedean spiral comparison", echo=FALSE, message=FALSE, warning=FALSE}
...
With this method, comparing the Static Spiral Test and the Dynamic Spiral Test, it is easier to categorize the patients (see Figure \ref{fig:figs13}). The results are more distinct.

```{r calculating distance between Test 0 and Test 1, fig.cap="\\label{fig:figs13} SST-DST comparison", echo=FALSE, message=FALSE, warning=FALSE}
...
### Tests inconsistency \label{sect_test_inconsistency}
...
As seen in the *Data Cleaning - Calibrating the samples* (see \ref{section_data_cleaning}) section, the records were not on the same scale and sometimes not on the same starting point.
...
In this section I define the predition models and different algorithms.
...
I picked the random forest model (`randomForest`). The k-nearest neighboors and the linear regression were not approriate for the data gathered and computed. I would do the weighted mean between the two predictions.
...
Parkinson’s disease" by Muhammed Erdem Isenkula, Betul Erdogdu Sakarb, Olcay Kursuna :
