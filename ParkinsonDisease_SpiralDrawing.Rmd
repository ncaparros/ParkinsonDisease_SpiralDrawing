...
```
We can indeed see that even if most ratios are around 1.3, some outliers are around 2.

```{r}
completeDf %>% filter(TestID == 0 & Timestamp == 0) %>% ggplot() + geom_point(aes(X,Y, color=isPwp))
```

```{r calibrating X depending on ratio}
