...
completeCalibratedDf <- completeCalibratedDf %>% mutate(Vt = NA, Dist = NA)
AnalyseDf <- completeCalibratedDf

for(index in seq(1, by=1, length=nrow(patients))){
  
  id <- patients[index,]$patientID
  
  temp0 <- completeCalibratedDf %>% 
    filter(patientID == id & 
             TestID == 0) %>% 
    arrange(Timestamp)
  
  temp1 <- completeCalibratedDf %>% 
    filter(patientID == id & 
             TestID == 1) %>% 
    arrange(Timestamp)
  
  if(nrow(temp0) > 0 & nrow(temp1) > 0){
