...
patients <- patients %>% mutate(areaT0 = NA, areaT1 = NA, areaT2 = NA, sdT0 = NA, sdT1 = NA, sdT2 = NA)
...
for(test in seq(0, by=1, length=2)){
 ...
 patients %>% ggplot() + geom_histogram(aes(areaT0, fill=isPwp, color=patientID), stat="bin") 
  patients %>% ggplot() + geom_histogram(aes(areaT1, fill=isPwp, color=patientID), stat="bin") 

```

This approach did not seem very conclusive. 

```{r calculating distance between Test 0 and Test 1}

#Initialisation of the values : total area difference between the drawing and the Archimedean spiral, and the standard deviation of the distance between the drawing and the spiral.
patients <- patients %>% mutate(areaT0_T1 = NA)
...
patients %>% ggplot() + geom_histogram(aes(areaT0_T1, fill=isPwp, color=patientID), stat="bin") 
...
times <- times %>% 
  mutate(difft = time1 - time0) %>% 
  select(difft, patientID)
