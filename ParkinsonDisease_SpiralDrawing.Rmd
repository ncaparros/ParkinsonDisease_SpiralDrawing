...
The motor symptoms of the Parkinson's disease (parkinsonian syndrome) are caused by the death of cells, more precisely dopaminergic neurons, in the *substantia nigra* (a region of the midbrain, see Figure 1, left). The *substantia nigra* is a structure divided into to parts : the *pars reticula* and the *pars compacta* (see Figure 1, left). It is the part of the brain that plays an important role in reward-seeking, learning and movement. 
...
```{r archimedean spiral, echo=FALSE, fig.align="center", fig.cap="Archimedean spirals"}
#####
b1 <- 10.61
 t1 <- seq(0,6*pi, length.out=1100)
 x1 <- b1*t1 * (cos(t1))
 y1 <- b1*t1 * -(sin(t1))
 
 #data frame containing the values (x,y) of the Archimedean spiral
 archimedeanSpiral = data.frame(x1,y1)
 
 #Ploting the Archimedean spiral and the points used to calibrate the samples.
 plot1 <- archimedeanSpiral %>%
   ggplot() +
   geom_path(aes(x1,y1)) +
   xlab("x")+
   ylab("y")+
...
#####
 ...
 theta = pi/4
 #####
 angle.t <- seq(0, theta, length.out=100)
 angle.x <- 5*cos(angle.t)
 angle.y <- 5*sin(angle.t)
 ...
 plot2 <- ggplot() + 
   ...
   #a when t=0
   geom_path(aes(c(0:a),0),col="#FFA434", size=1.5) +
   geom_text(aes(15,3),label="a", col="#FFA434")+
   ...
   geom_path(aes(seq(0,
                 ...
                 seq(0,
                     ...
                     length.out = 2)), 
                 col="#FFA434", size=1.5)+
  geom_text(aes(5,10),label="a*t", col="#FFA434")+
   
   #b when t=pi/4
   geom_path(aes(seq(a*cos(theta),
                     a*cos(theta)+b*theta*cos(theta),
                     length.out = 2),
                 seq((a*sin(theta)),
                     a*sin(theta)+b*theta*sin(theta), 
                     length.out = 2)),
                 col="#006837", size=1.5) + 
  geom_text(aes(20,25),label="b*t", col="#006837")+

   #t = pi/4
  geom_curve(aes(x = 7, y = 0, xend = 7*cos(theta), yend = 7*sin(theta)),
             color = "#2F86E0",
             size=1,
             arrow = arrow(type = "open", length = unit(7,"points")))+
    geom_text(aes(9,6),label="t", col="#2F86E0")+
...
```{r checking timestamps, echo=FALSE, message=FALSE, warning=FALSE}
