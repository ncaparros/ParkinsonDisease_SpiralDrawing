...
The terms anotated with an asterisk were explained or more detailled in the glossary at the end of the report.
...
![Lateral cross-section of the brain (source : http://www.neuroconvention.com/)](img/brain_pd.jpg){width=50%, heigth=50%} ![*Substancia nigra* differences between a healthy brain and a Parkinson's brain (source : https://scienceofparkinsons.com/)](img/sn.jpg){width=50%, heigth=50%}
...
![Wacom Cintiq 12WX graphics, source : https://www.bhphotovideo.com/](img/wacom.jpg){width=50%, heigth=50%}
...
This would probably alter the accuracy of the future predictions. The original study based its analysis on the combination of tests 0 and 1, and though every patient took the first test (Static Spiral Test), some tests 1 (Dynamic Spiral Test) were missing from the dataset. I had to base a prediction method on the sole first test for the patients who only took this one.
...
subTest2Parkinsons  <- groupedPatientTestDf %>% 
  filter(TestID == 2 & isPwp==TRUE) %>% 
  ungroup() %>% 
  select(patientID) %>% 
  unique()

subControls <- groupedPatientTestDf %>% 
  filter(isPwp==FALSE & !patientID %in% subTest2Ctrl$patientID) %>% 
  ungroup() %>% 
  select(patientID) %>% 
  unique()

subParkinsons <- groupedPatientTestDf %>% 
  filter(isPwp==TRUE & !patientID %in% subTest2Parkinsons$patientID) %>% 
  ungroup() %>% 
  select(patientID) %>% 
  unique()

nrow(subTest2Ctrl) + nrow(subTest2Parkinsons) + nrow(subControls) + nrow(subParkinsons) == nrow(patients)

#Getting random samples for each sub-data set
testIndexT2Ctrl <- sample(1:nrow(subTest2Ctrl), floor(nrow(subTest2Ctrl)*0.1)+1) 
testIndexT2Parkinsons <- sample(1:nrow(subTest2Parkinsons), floor(nrow(subTest2Parkinsons)*0.1)+1) 
testIndexCtrl <- sample(1:nrow(subControls), floor(nrow(subControls)*0.1)+1) 
testIndexParkinsons <- sample(1:nrow(subParkinsons), floor(nrow(subParkinsons)*0.1)+1) 

testing_patients <- subTest2Ctrl[testIndexT2Ctrl,]
testing_patients <- rbind(testing_patients, subTest2Parkinsons[testIndexT2Parkinsons,])
testing_patients <- rbind(testing_patients, subControls[testIndexCtrl,])
testing_patients <- rbind(testing_patients, subParkinsons[testIndexParkinsons,])
...
```{r calculating distance between Archimedean Spiral and drawing0}

#Initialisation of the values : total area difference between the drawing and the Archimedean spiral, and the standard deviation of the distance between the drawing and the spiral.
patients <- patients %>% mutate(areaT0 = NA, areaT1 = NA, sdT0 = NA, sdT1 = NA)
...
plotT0Archi <- plotComparingTestsWithArchimedeanSpiral + geom_histogram(aes(areaT0, fill=isPwp, color=patientID), stat="bin") 

plotT1Archi <- plotComparingTestsWithArchimedeanSpiral + geom_histogram(aes(areaT1, fill=isPwp, color=patientID), stat="bin") 

plotSdT0Archi <- plotComparingTestsWithArchimedeanSpiral + geom_histogram(aes(sdT0, fill=isPwp, color=patientID), stat="bin") 

plotSdT1Archi <- plotComparingTestsWithArchimedeanSpiral + geom_histogram(aes(sdT1, fill=isPwp, color=patientID), stat="bin") 

ggarrange(plotT0Archi, plotT1Archi, plotSdT0Archi,plotSdT1Archi, ncol=2, nrow=2, common.legend = TRUE)

```

This approach did not seem very conclusive. Even if the results of the healthy patient were close, many patients with Parkinson's disease had similar results. It seemed more appropriate to compare instead the drawings of the first two tests and to compute their area difference. I also computed the time difference between those two tests.

```{r calculating distance between Test 0 and Test 1}
...
patients <- patients %>% mutate(areaT0_T1 = NA, sdT0_T1 = NA)
  ...
  for(p in seq(1, by=1, length=nrow(patients))){
    ...
    if(nrow(temp_df0)>0 & nrow(temp_df1)>0){
      ...
      temp_df <- temp_df %>% mutate(dist = sqrt((X.x-X.y)^2+(Y.x-Y.y)^2)) %>% select(X.x,Y.y,dist)
...
plotAreaT0_T1 <- plotT0_T1 + geom_histogram(aes(areaT0_T1, fill=isPwp, color=patientID), stat="bin") 

plotSdT0_T1 <- plotT0_T1 + geom_histogram(aes(sdT0_T1, fill=isPwp, color=patientID), stat="bin")

plotDiffTimesT0_T1 <- plotT0_T1 + geom_histogram(aes(difft1_t0, fill=isPwp, color=patientID), stat="bin")

ggarrange(plotAreaT0_T1,plotSdT0_T1, plotDiffTimesT0_T1, ncol=2, nrow=2, common.legend = TRUE)
...
### Instantaneous velocity (Static and Dynamic Test) analysis

The distance between two consecutive samples gave the instantaneous velocity at time $t$ ($V_t$).

$$V_t = \sqrt{(x_t-x_{t-1})^2 + (y_t-y_{t-1})^2}$$
```{r calculation instantaneous velocity}

completeCalibratedDf <- completeCalibratedDf %>% 
  mutate(Vt = NA, Dist = NA)
    ...
    for(index in seq(1, by=1, length=nrow(patients))){
      ...
      if(nrow(temp)>0){
        ...
        for(row in seq(2, by=1, length=nrow(temp))){
          ...
          temp[row,]$Vt <- sqrt((temp[row,]$X - temp[row-1,]$X)^2 + 
                                  (temp[row,]$Y - temp[row-1,]$Y)^2)
          
          temp[row,]$Dist <- temp[row - 1,]$Dist + temp[row,]$Vt
        }
...
```
Since we did not have the unit of the time (Timestamp), it was not possible to measure the value of the velocity, nor to analyse the speed at which the patient was drawing. But it was possible to compare the acceleration (change of velocity) for the Static and Dynamic Spiral Tests.

### Acceleration (Static and Dynamic Test) analysis

The acceleration at time $t$ ($A_t$) would be : $$A_t = V_t-V_{t-1}$$

```{r Measuring acceleration}
AnalyseDf <- AnalyseDf %>% 
  mutate(A = NA)

AnalyseDfAcc <- data.frame()
    ...
    for(index in seq(1, by=1, length=nrow(patients))){
      ...
      if(nrow(temp)>0){
        ...
        for(row in seq(2, by=1, length=nrow(temp))){
          ...
          temp[row,]$A <- temp[row,]$Vt - temp[row-1,]$Vt
          
        }
        
        AnalyseDfAcc <- rbind(AnalyseDfAcc, temp)
      }
    }
}
```
...
This test would give important information about the hand tremor of the patient. If the pen was to touch the screen, or move a lot over the tablet, the patient would probably have the Parkinson's disease. To quantify this, I computed the number of points on the screen (basically every point where Z equal to 0) and the total distance on the X and Y axis of the pen.

```{r computing values for Test2}
...
couplesTestsPatients <- df %>% select(patientID,TestID) %>% group_by(patientID, TestID) %>% unique()
PatientsT0 <- couplesTestsPatients %>% filter(TestID == 0)
PatientsT1 <- couplesTestsPatients %>% filter(TestID == 1)
PatientsT2 <- couplesTestsPatients %>% filter(TestID == 2)

#Are there any patients who took the SST but neither the DST nor the SOCP?
PatientsT0 %>% filter(!patientID %in% PatientsT1$patientID & !patientID %in% PatientsT2$patientID) %>% nrow() > 0

#Are there any patients who took the SST and the DST but not the SOCP?
PatientsT0 %>% filter(patientID %in% PatientsT1$patientID & !patientID %in% PatientsT2$patientID) %>% nrow() > 0

#Are there any patients who took the SST and the SOCP but not the DST?
PatientsT0 %>% filter(!patientID %in% PatientsT1$patientID & patientID %in% PatientsT2$patientID) %>% nrow() > 0
...
#Are there any patients who took the DST but neither the SST nor the SOCPI?
PatientsT1 %>% filter(!patientID %in% PatientsT0$patientID & !patientID %in% PatientsT2$patientID) %>% nrow() > 0

#Are there any patients who took the DST and the SOCP but not the SST?
PatientsT1 %>% filter(patientID %in% PatientsT2$patientID & !patientID %in% PatientsT0$patientID) %>% nrow() > 0
PatientsT2 %>% filter(patientID %in% PatientsT1$patientID & !patientID %in% PatientsT0$patientID) %>% nrow() > 0

#Are there any patients who took the SOCP but neither the SST nor the DST?
PatientsT2 %>% filter(!patientID %in% PatientsT0$patientID & !patientID %in% PatientsT1$patientID) %>% nrow() > 0
```
...
* Prediction on the Static Spiral Test compared to the Archimedean Spiral ($\hat{Y}_{SST\sim Arch}$)
...
$$\hat{Y} = \overline{\hat{Y}_{SST\sim Arch} + 2*\hat{Y}_{SST\sim DST} + 3*\hat{Y}_{SOCPT}}$$
When there was a missing missing predictions, they were removed from the equation.

### Chosing the prediction algorithms

I picked two algorithms, the linear regression model (`lm`) and the random forest model (`randomForest`). The k-nearest neighboors was not approriate for the data gathered and computed. I would then do the mean between the two predictions.

The model would become :

$$\hat{Y} = \overline{\frac{\hat{Y}_{SST\sim Arch, lm} + \hat{Y}_{SST\sim Arch, rf}}{2} + 2*\frac{\hat{Y}_{SST\sim DST, lm} + \hat{Y}_{SST\sim DST, rf}}{2}+ 3*\frac{\hat{Y}_{SOCPT,lm}+\hat{Y}_{SOCPT,rf}}{2}}$$
...
trainingSet <- left_join(trainingSet, patients %>% select(-isPwp), by="patientID")
testingSet <- left_join(testingSet, patients %>% select(-isPwp), by="patientID")
```
...
* Static Spiral Test compared to the Archimedean Spiral ($\hat{Y}_{SST\sim Arch}$) : area difference between the test 0 and the Archimedean spiral (`areaT0`) and its standard deviation (`sdT0`)
...
```{r fitting models}
#Static Spiral Test compared to the Archimedean Spiral
#LM :
fit_SST_Arch_lm <- lm(isPwp ~ areaT0 + sdT0, data=trainingSet)
yhat_SST_Arch_lm <- predict(fit_SST_Arch_lm, testingSet)

#random forest
fit_SST_Arch_rf <- randomForest(isPwp ~ areaT0 + sdT0, data=trainingSet)
yhat_SST_Arch_rf <- predict(fit_SST_Arch_rf, testingSet)


#Static Spiral Test compared to the Dynamic Spiral Test
#LM :
fit_SST_DST_lm <- lm(isPwp ~ areaT0_T1 + sdT0_T1 + difft1_t0, data=trainingSet)
yhat_SST_DST_lm <- predict(fit_SST_DST_lm, testingSet)

#random forest
fit_SST_DST_rf <- randomForest(isPwp ~ areaT0_T1 + sdT0_T1 + difft1_t0, data=trainingSet, na.action = na.omit)
yhat_SST_DST_rf <- predict(fit_SST_DST_rf, testingSet)


#Stability On Certain Point Test
#lm
fit_SOCPT_lm <- lm(isPwp ~ Z0_T2 + DistT2, data=trainingSet)
yhat_SOCPT_lm <- predict(fit_SOCPT_lm, testingSet)

#random forest
fit_SOCPT_rf <- randomForest(isPwp ~ Z0_T2 + DistT2, data=trainingSet, na.action=na.omit)
yhat_SOCPT_rf <- predict(fit_SOCPT_rf, testingSet)


#Computing the predictions
yhatDf <- data.frame(yhat_SST_Arch_lm,
                    yhat_SST_Arch_rf,
                    yhat_SST_DST_lm,
                    yhat_SST_DST_rf,
                    yhat_SOCPT_lm,
...
yhatDf <- yhatDf %>% mutate(yhat_SST_Arch = (yhat_SST_Arch_lm + yhat_SST_Arch_rf)/2,
                            yhat_SST_DST = (yhat_SST_DST_lm + yhat_SST_DST_rf)/2,
                            yhat_SOCPT = (yhat_SOCPT_lm + yhat_SOCPT_rf)/2) %>%
  select(yhat_SST_Arch,yhat_SST_DST,yhat_SOCPT)

yhatDf <- yhatDf %>% rowwise() %>% mutate(yhat = weighted.mean(c(yhat_SST_Arch,yhat_SST_DST,yhat_SOCPT), c(1,2,3), na.rm=TRUE))
```
